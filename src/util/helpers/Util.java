...
public class Util {
	...
	/**
	 * Verifica a versão do Python instalada, para poder utilizar do Pytesseract -
	 * Pré requisito para o OCR
	 */
	public static void checkPythonVersion() {
		try {
			if (getOs().contains(WINDOWS_OS)) {
				Process p = Runtime.getRuntime().exec("python --version");
				BufferedReader in = new BufferedReader(new InputStreamReader(p.getInputStream()));
				String line = in.readLine();
				if (line != null && line.matches(".*\\s3.*\\..*\\..*")) {
			...
			} else if (getOs().equalsIgnoreCase(LINUX_OS)) {
				String line;
	...
	/**
	 * Evento de Click, caso pressionado um pré-requisito na tela, realiza ações de
	 * Enable(True)
	 *
	 * @param table   JTable
		...
		try {
			table.addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e) {
					...
					/*
					 * Remove o focus e as linhas selecionadas da tabela e trasnfere para o campo de
					 * procura apois 20 mili segundos
					 */
					if (table.hasFocus()) {
	 ...
	 *
	 * <p>
	 * Se o <strong>PRIMEIRO</strong> botão/algo estiver visivel e a cor:
	 * {@code String/Color != null || !empity} o botão recebe uma cor.
	 * </p>
	 *
	 * <p>
	 * Caso tenha uma cor secundaria, para <strong>demais botões</strong> estes
	 * receberão, caso contrario, uma cor <strong>Default</strong>(new
	 * Jbutton().getBackgroundColor)
	 * </p>
	 *
	 * <p>
	 * <strong>ESTES BOTÕES RECEBERÃO AS CORES COM A RESPECTIVA ORDEM DE
	 * ADIÇÃO</strong>
	 * </p>
	 *
	 ...
	 * @param btns             Array[]
	 */
	public static void mudarCorBotao(boolean visible, boolean defaultBackgroud, String strMainColor,
			String strRestColors, Color mainColor, Color restColors, JButton... btns) {
		try {
			...
			try {
				System.out.println(
						e.getClass().getMethod("mudarCorBotao", Util.class, Color.class, String.class, Array.class));
	...
	/**
	 * Verifica o sistema operacional para executar um comando que verifica o
	 * Desktop, e roda na terminal um comando, pegando o retorno e inserindo no
	 * JFileChooser como padrão de abertura
	 *
	 * @return String user directory
	 */
	private static String getDesktopPath() {
		try {
			if (getOs().equalsIgnoreCase(LINUX_OS)) {
				String path;
	...
	// Os Calculos Abaixo foram feitos de DUAS MANEIRAS, TimeUnit, e Representações
	// a 'mão'
	// Ambos estão iguais
...
//    long seconds = TimeUnit.MILLISECONDS.toSeconds(remainingSecondsInMillis);
	...
	/**
	 * Método que calcula diferença de dias, horas, minutos, segundos, podendo
	 * utilizar os getters apartir desse método
	 *
	 * @param diff long
	 */
