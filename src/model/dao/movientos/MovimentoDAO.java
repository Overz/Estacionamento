...
public class MovimentoDAO implements BaseDAO<MovimentoVO> {
	...
	public ArrayList<MovimentoVO> consultar(String... values) {
		...
		if (ConstHelpers.FLAG == 0) {
			...
			qry = "select * from movimento movi" + "  left join ticket t on movi.idTicket = t.id "
					+ "  left join contrato con on movi.idContrato = con.id "
					+ "  left join cliente cli on con.idCliente = cli.id = con.id " + " where t.n_ticket like '%"
					+ values[0] + "%' " + " or con.n_cartao like '%" + values[0] + "%' ;";
	...
	public boolean excluirPorID(int id) {
		String qry = ConstHelpers.FLAG == 0
				? "delete m from movimento m " + "inner join contrato con on m.idContrato = con.id "
						+ "inner join cliente cli on con.idCliente = cli.id "
						+ "inner join carro car on cli.idCarro = car.id "
						+ "inner join endereco e on cli.idEndereco = e.id " + "where m.id = ?"
	...
	public boolean excluirPorContratoId(int id) {
		String qry = "delete from movimento where idContrato = ?";
